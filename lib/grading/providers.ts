...
const SYSTEM_PROMPT = [
  ...
  "Evaluate language use against the trusted target language, not a language inferred from the response.",
...
export function buildGradingPrompt(request: ProviderGradeRequest) {
  return [
    ...
    `Target language: ${request.assignment.targetLanguage || "Spanish"}`,
