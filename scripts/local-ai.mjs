...
async function ensureSchema(db) {
  ...
  await exec(db, "CREATE TABLE IF NOT EXISTS users (email TEXT PRIMARY KEY, role TEXT NOT NULL DEFAULT 'student', created_at INTEGER NOT NULL, is_paid INTEGER NOT NULL DEFAULT 0)");
...
async function smoke() {
  ...
  const after = await exec(db, "SELECT grade, feedback FROM submissions WHERE id = ?", [submissionId]);
  if (before.rows[0]?.grade !== after.rows[0]?.grade || before.rows[0]?.feedback !== after.rows[0]?.feedback) {
    throw new Error("AI route modified final grade or feedback.");
  }
  const latest = await fetch(`http://localhost:3000/api/submissions/${submissionId}/ai-grade`);
  if (!latest.ok) throw new Error("Could not retrieve latest AI suggestion.");
  const second = await fetch(`http://localhost:3000/api/submissions/${submissionId}/ai-grade`, { method: "POST" });
  console.log(`suggestion: schema-valid`);
  console.log(`final grade unchanged: yes`);
  console.log(`latest suggestion retrieval: ${latest.status}`);
  console.log(`second immediate request: ${second.status}`);
}
