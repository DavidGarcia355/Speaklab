...
| `AI_BULK_GRADING_ENABLED` | Enables assignment-wide AI suggestions separately from single-submission grading. A server-owned, idempotent batch snapshots eligible ungraded submissions, checkpoints each attempt, and stages scores, rubric breakdowns, and feedback for teacher review. Nothing is written to student-visible grades until the teacher explicitly saves the reviewed batch. | Optional rollout flag | Server | Defaults disabled. |
